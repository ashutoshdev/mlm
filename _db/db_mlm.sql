...
--
-- Host: localhost
-- Generation Time: Aug 05, 2014 at 07:19 PM
...
CREATE TABLE IF NOT EXISTS `company_transaction_details` (
  ...
  `note` text NOT NULL,
  PRIMARY KEY (`transaction_id`,`transaction_date`,`item_id`),
  KEY `item_id` (`item_id`)
...
(3, 1, 0, 2, 0, 'user1', 'user1', 'user1', '0000-00-00');
...
ALTER TABLE `company_transaction_details`
  ...
  ADD CONSTRAINT `company_transaction_details_ibfk_2` FOREIGN KEY (`item_id`) REFERENCES `item_master` (`item_id`) ON DELETE CASCADE ON UPDATE CASCADE;
